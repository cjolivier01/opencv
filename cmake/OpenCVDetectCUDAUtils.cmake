macro(ocv_check_for_nvidia_video_codec_sdk cuda_toolkit_dirs)
  macro(ocv_cuda_SEARCH_NVIDIA_VIDEO_CODEC_HEADER _filename _result _include_dir)
    find_path(_header_result
      ${_filename}
      PATHS ${cuda_toolkit_dirs} /usr /usr/local
      PATH_SUFFIXES include include/ffnvcodec ffnvcodec
      NO_DEFAULT_PATH
      )
    if("x${_header_result}" STREQUAL "x_header_result-NOTFOUND")
      ...
      set(${_result} 1)
      set(${_include_dir} "${_header_result}" CACHE PATH "NVIDIA Video Codec SDK include directory" FORCE)
    endif()
    unset(_header_result CACHE)
  endmacro()
  if(WITH_NVCUVID)
    ocv_cuda_SEARCH_NVIDIA_VIDEO_CODEC_HEADER("nvcuvid.h" HAVE_NVCUVID_HEADER CUDA_NVCUVID_INCLUDE_DIR)
    if(NOT HAVE_NVCUVID_HEADER)
      ocv_cuda_SEARCH_NVIDIA_VIDEO_CODEC_HEADER("dynlink_nvcuvid.h" HAVE_DYNLINK_NVCUVID_HEADER CUDA_NVCUVID_INCLUDE_DIR)
    endif()
    # make sure to have both header and library before enabling
    if(HAVE_NVCUVID_HEADER OR HAVE_DYNLINK_NVCUVID_HEADER)
      find_cuda_helper_libs(nvcuvid)
      if(CUDA_nvcuvid_LIBRARY)
        set(HAVE_NVCUVID 1)
        message(STATUS "Found NVCUVID: ${CUDA_nvcuvid_LIBRARY} ${CUDA_NVCUVID_INCLUDE_DIR}")
      else()
        if(WIN32)
      ...
      message(STATUS "NVCUVID: Header not found, WITH_NVCUVID requires Nvidia decoding library header nvcuvid.h or dynlink_nvcuvid.h")
    endif()
  endif()

  if(WITH_NVCUVENC)
    ocv_cuda_SEARCH_NVIDIA_VIDEO_CODEC_HEADER("nvEncodeAPI.h" HAVE_NVCUVENC_HEADER CUDA_NVCUVENC_INCLUDE_DIR)
    if(${HAVE_NVCUVENC_HEADER})
      ...
      if(CUDA_nvencodeapi_LIBRARY OR CUDA_nvidia-encode_LIBRARY)
        set(HAVE_NVCUVENC 1)
        message(STATUS "Found NVCUVENC: ${CUDA_nvencodeapi_LIBRARY} ${CUDA_nvidia-encode_LIBRARY} ${CUDA_NVCUVENC_INCLUDE_DIR}")
      else()
        if(WIN32)
      ...
      message(STATUS "NVCUVENC: Header not found, WITH_NVCUVENC requires Nvidia encoding library header nvEncodeAPI.h")
    endif()
  endif()
endmacro()
